...
If the current sign is $\sigma\in\lbrace +1,-1\rbrace $ and the current magnitude is $x$, then:
...
$$
E\subseteq\lbrace 2,3,4\rbrace 
$$
...
For $g\in\lbrace 0,1,2\rbrace $, let $C_g(p)$ denote the state in which
...
> **Lemma 1 (One-sided cleanup).** For every $N\ge 1$ and every $E\subseteq\lbrace 2,3,4\rbrace $, the one-sided walk reaches a canonical state $C_g(p)$ satisfying
...
The checker exhaustively verifies all $1\le N<100$ and all eight subsets of $\lbrace 2,3,4\rbrace $. The largest overshoot is exactly $23$.
...
The normalized state associated with $B_r$ is
...
- $\sigma$ is the current sign;
- $f$ is the leading-prefix offset;
- $e$ is the other-prefix offset;
- $u$ is the current-magnitude offset;
- $E_A$ is the set of leading-side extras measured relative to $L$;
- $E_B$ is the set of other-side extras measured relative to $Q$.
...
$$
(u,E_A)\in
\lbrace (-2,\varnothing),(-1,\varnothing),(0,\varnothing),(2,\lbrace 2\rbrace ),(3,\lbrace 3\rbrace )\rbrace ,
$$
...
$$
E_B\in
\lbrace \varnothing,\lbrace 2\rbrace ,\lbrace 3\rbrace ,\lbrace 4\rbrace ,\lbrace 2,3\rbrace \rbrace .
$$
...
The least same-side candidate and the $\text{root-}(r+1)$ landing differ from $L$ by constants depending only on
...
Thus the next move from every invariant $B_r$ is the $\text{root-}(r+1)$ crossing.
...
- $D$ is marked used;
- the only other initially marked positive coordinates are $E_B\subseteq\lbrace 2,3,4\rbrace $.
...
$$
\mathcal C=\bigl\lbrace (28,2),(29,1),(29,2),(30,0),(30,1),(30,2)\bigr\rbrace .
$$
...
1. **Lower roots are unavailable.** The raw $\text{root-}r_0$ landing lies at or below the opposite filled prefix. Hence every smaller root is unavailable as well.

2. **The old crossing is exact when available.** If the $\text{root-}(r_0+1)$ landing is unused, it is the least opposite-side candidate.

3. **The next-root landing is genuinely unused when the old crossing is unavailable.** The raw $\text{root-}(r_0+2)$ landing lies above every used point on the opposite side. Thus it is the actual next opposite candidate, not merely a lower bound for one.
...
At $S_0$, the $\text{root-}(r_0+12)$ landing is therefore delayed. The checker verifies ten same-side moves with these stronger conditions at every step:

- the $\text{root-}(r_0+11)$ reflection lies inside the inactive filled prefix;
- the $\text{root-}(r_0+12)$ landing lies above every used inactive point;
...
- the $\text{root-}(r_0+12)$ landing is unused;
...
- every $\text{root-}(r_{j+1}+2)$ candidate is also the corresponding base candidate translated by $10j$;
- every $\text{root-}(r_{j+1}+1)$ reflection is the corresponding base reflection shifted downward by $10j$.

Hence all ten comparisons have exactly the same outcome as in the finite certificate. The active side gains another translation by 10, the inactive side remains fixed, and the next move is the $\text{root-}(r_{j+1}+2)$ exit. This establishes the induction.
...
$$
\boxed{
B_{100}=(0,+,0,0,2,\lbrace 2\rbrace ,\varnothing)
}
$$
...
- $f=e=0$;
- $u=2$;
- $E_A=\lbrace 2\rbrace $ and $E_B=\varnothing$.
...
- Section 7 proves that the next move is the $\text{root-}(r+1)$ crossing.
