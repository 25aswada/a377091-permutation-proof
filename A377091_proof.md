...
**Lemma 1.** For every \(N\ge1\) and every \(E\subseteq\{2,3,4\}\), the one-sided walk reaches a canonical state \(C_g(p)\) satisfying
...
\[
(r\bmod10,\sigma,f,e,u,E_A,E_B),
\]

where \(\sigma\) is the current sign and the extra sets are measured relative to \(L\) and \(Q\).
...
**Lemma 2.** For a fixed normalized boundary state and checkpoint type, the normalized phase output is the same for every
...
```bash
python A377091_proof_checker_v4.py
```
